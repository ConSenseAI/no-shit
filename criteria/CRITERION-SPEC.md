...
### 7.1 Calibration gate (draft → candidate)

Before any pipeline runs, the criterion's text is calibrated against independent readers. **Two components, both required**, each against a target **finalized before adjudication begins** — one metric, stated thresholds; a gate whose metric is chosen after the results are known is not a gate (the same stated-in-advance discipline as the §7.2 kill thresholds). Adjudication runs under `ADJUDICATION.md` (blinded packets, rule-excerpt-only reading, divergence logging, roster/record/publication standards).

**(a) Reliability — full corpus, lineage-diverse, human or model.** ≥2 independent, blinded adjudicators over the full public corpus. The **counted panel** MUST be mutually lineage-unrelated, and every counted adjudicator MUST be independent of the criterion's authoring lineage(s) — an author-lineage rater MAY run alongside as a diagnostic but does not count toward the gate. Model adjudicators are permitted and, for full-corpus reliability, are in practice the stronger instrument (affordable full coverage, procedural discipline, *chooseable* lineage diversity — finding T-13); "unrelated lineages" carries the enclave-pool sense: unrelated training lineages, not API vendors. Targets: the registered aggregate agreement (e.g., Cohen's/Fleiss' κ ≥ 0.8) AND a hard-subset floor (e.g., ≥80% exact-verdict) on the `boundary` + `adversarial` categories pooled — aggregate agreement over a mostly-easy corpus can mask exactly the boundary ambiguity this gate exists to catch (the `no-subscription-trap` dry-run measured 92.5% overall alongside 40% on its engineered-hard probes). The manifest records both measured agreements and `corpus_covered` (SHOULD be 1.0).

**(b) Human anchor — stratified sample.** ≥2 independent **humans** over a stratified sample: every `boundary` and `adversarial` row, every row the criterion's calibration records flag as a watch-item, plus a stated random slice of the remainder (composition and selection seed recorded in the calibration record). The criteria assert facts about human experience of software, and model raters share training corpora and alignment conventions — a shared misreading of a perception-laden predicate would survive any all-model panel. The anchor tests construct validity exactly where judgment has been shown fragile, at a human workload of one sitting rather than the full corpus. Target: the registered sample floor (e.g., ≥80% exact-verdict), measured as inter-human agreement on the sample; agreement between the human panel and the reliability panel is recorded alongside — a human split the reliability panel sailed through is a finding, not noise.

Rationale (T-13): reliability and anchoring are different jobs. Reliability asks whether the rule text pins verdicts for independent competent readers — cross-lineage model panels now do this better than two same-culture humans. Anchoring asks whether the pinned verdicts track the thing the criteria exist to protect — and an instrument is not calibrated entirely against copies of itself. If calibrated readers cannot agree, the checks are under-operationalized and **no downstream audit ensemble will do better**; a criterion that fails either component goes back to §4 — the fix is sharper checks, not better raters.
...
validation:                        # §7 — nulls until measured; gates status transitions
  calibration:                     # §7.1 (0.7.0, T-13) — two components; targets pre-registered BEFORE adjudication begins
    reliability:                   # (a) full corpus; counted panel: >=2 blinded adjudicators, mutually lineage-unrelated, authoring-lineage-independent (human or model)
    ...
    human_anchor:                  # (b) >=2 independent humans; stratified sample (all boundary+adversarial rows + watch-items + stated random slice)
      target: { sample: null, registered_at: null }   # e.g. ">=80% exact-verdict on the stratified sample"
      adjudicators: 0
      sample_size: null
      agreement_sample: null       # inter-human, on the sample
      agreement_vs_reliability: null
...
**To reach `candidate`:** all of the above, plus the **calibration gate passed — both components (reliability + human anchor)** — against pre-registered targets (§7.1).
...
- **0.7.0** (2026-07-06) — **T-13 — calibration-gate recomposition** (§7.1): the human-calibration gate becomes the **calibration gate**, two components. **(a) Reliability** — full public corpus, ≥2 blinded adjudicators, **human or model**; the counted panel is mutually lineage-unrelated and independent of the authoring lineage(s) (author-lineage raters run as diagnostics only); thresholds unchanged (aggregate κ + hard-subset floor). **(b) Human anchor** — ≥2 independent humans over a stratified sample (all boundary+adversarial rows, all watch-items, a stated random slice) against the registered sample floor; human-vs-reliability agreement recorded. Reasoning: reliability (does the text pin verdicts for independent readers?) is now done better by cross-lineage model panels — affordable full coverage, procedural discipline, chooseable diversity — while anchoring (do the verdicts track human experience, the thing the criteria protect?) cannot be delegated to the rater class being validated. Stated plainly: this was decided **after** the first cross-vendor panel (2026-07-06) returned 152/152 — so that panel, which also included an author-lineage rater, is **not counted** toward any gate; the recomposition binds only rounds run after it, and registered thresholds carry unchanged. Manifest shape: `validation.human_calibration` → `validation.calibration.{reliability, human_anchor}` (breaking rename in the T-4 zero-attestation window, as 0.4.0's was; all three criteria migrate same-day). §2.3 and Appendix B restated; Appendix A updated; ADJUDICATION.md 0.2.0 carries the operational side.
