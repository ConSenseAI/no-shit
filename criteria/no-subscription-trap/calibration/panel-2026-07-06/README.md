# Cross-vendor calibration panel — 2026-07-06 (run 3)

Published evidence for the run recorded in `../../calibration-dryrun.md` (run 3) and SPEC §7. **Engineering dry-run — not the formal §7.1 gate**; `human_calibration` stays 0.
...
Coordinator: project owner. Criterion version adjudicated: **0.1.8** (current 0.1.9 differs only by the panel-flag row sharpenings in the changelog — no decision changes). Blinding is honor-system plus packet construction; attestations are in the sheets verbatim.

## Results

n = 42 (37 corpus rows + 5 ambiguity probes), coverage 1.00. Inter-rater exact 42/42, **Cohen's κ = 1.00**; hard subset (boundary+adversarial, n = 11) 11/11; both raters 42/42 vs. the authored key; zero underdetermined. Probes count in the aggregate, not the hard subset. Scored with `tools/adjudication/score.py`.
