...
**Supply survey as of:** 2026-07-08, targeted re-verifications 2026-07-10 (external-world facts in §5 are dated estimates, not standing truth)
...
| Adobe (hidden ETF, cancel obstruction) | filed 2024 · $150M settlement, **order entered 2026-04-10** (N.D. Cal.; $75M civil penalty + $75M free services) | nst core · ndp | remediation ordered by entered order: pre-enrollment ETF disclosure, >7-day-trial conversion reminders, easy cancel *(confirmed 2026-07-10, court record)* |
...
| "Genesis Tech" — FTC v. *Growthmind/Wisey* (4:26-cv-05232; the enterprise brand, not a defendant name — *corrected 0.1.2*) | filed 2026-06-02 · TRO + asset freeze 06-04 · active | nst · ndp | active litigation, no merits ruling *(confirmed, complaint + TRO read)* |
...
- **France:** the CNIL cookie-refusal-parity cluster — Google (€150M 2021; *corrected 0.1.1:* the 2025 **€325M** matter, SAN-2025-004, is **separate conduct** — Gmail inline ads + account-creation cookie consent, remediation ordered on a 6-month deadline — not banner refuse-parity), Facebook, Amazon, Microsoft, TikTok, Shein (€150M 2025) — the banner cluster remediated, which makes it double as a **remediation-verified clean-control set** for consent surfaces (§5.5). DGCCRF: Shein fake-discount fine (€40M, conduct reported still observable). And the survey's one true **post-termination-data enforcement anchor: Free/Freebox (CNIL, €300K) — reconditioned set-top boxes shipped to new customers still holding prior subscribers' photos** — a real-world `nli` check-6 shape beyond Alexa.
...
- **GDPR, the `no-lock-in` side:** named erasure cases exist (AEPD v. Google €10M — right-to-be-forgotten form obstruction; the Irish DPC's Twitter/X photo-ID-to-delete matter; SATS; Danske Bank), and the **EDPB's 2025 coordinated enforcement framework on erasure** (32 DPAs, 764 controllers) — *updated 0.1.2:* its report was **adopted 2026-02-10**, but the 9 formal investigations remain open and no framework-produced fine exists yet; the concrete fines the report cites predate it (Finland's ParkkiPate €70K, court-final 2023; the Dutch APG €6K). The framework's named focus areas — backup-erasure gaps, anonymisation-as-deletion — are corpus-shape input; the investigations stay the pipeline to watch. **Standalone GDPR Article 20 (portability) enforcement: zero DPA fines, confirmed as an absence** — the export-side gap is global, not an FTC quirk.
...
|---|---|---|---|---|
| `no-subscription-trap` (behavioral) | reachable: ~30 exhibit-anchored re-implementations + synthetics | **~30 products** — *corrected 0.1.1:* checks **5 and 6** now have real anchors (check 5: Wowcher pre-tick, ABCmouse buried-terms consent, TransUnion missing affirmative consent, AGCM Amazon PS12585 fined; check 6: AGCM eDreams PS12853, fined 2026 — trial→charge with no advance notice, decision read primary; *and 0.1.2:* **check 7 gained its first real anchors** — the CMA antivirus and console-trio commitments carry explicit renewal-reminder mandates (commitment strength). **Every check 1–7 is now real-anchored** at some strength | ~4–6 (Uber One; eDreams; UK Adobe probe; §312k reservoir pending own labeling) | reachable: platform-mandate class (store-scoped labels) + remediation-verified + construction |
...
- **`no-subscription-trap`:** the sealed violating set is roughly **half documented-real-anchored, half authored** — and the authored half is forced not by count but by *check coverage* (*narrowed 0.1.1, closed 0.1.2:* every check shape now carries a real anchor — auto-renew consent via Wowcher/ABCmouse/TransUnion/AGCM-Amazon, trial conversion via eDreams, and renewal reminders via the CMA antivirus/console commitment mandates; the authored half is now driven by count and record-strength diversity, not by any check having zero found supply). The clean set is attainable but every clean label carries a scope (store-mediated vs. web) and a date.
...
- §5's supply counts are dated estimates from a multi-pass web survey (2026-07-08) with per-claim confidence tags; enforcement dockets and datasets move. Counts are floors-of-knowledge, not floors-of-existence. Known soft spots, flagged inline: ~~the Adobe order's entry status~~ (**resolved 0.1.1:** entered 2026-04-10, verified against the court record); Italian AGCM matters — *partially resolved 0.1.1:* the eDreams (PS12853) and Amazon Subscribe & Save (PS12585) decisions were read from primary bollettino PDFs; Deghi (PS13027) remains bot-blocked, secondary-only; the German §312k 58% figure is a consumer-association sweep, not an adjudicated count; EDPB erasure-framework outcomes still pending.
...
- **0.1.2** (2026-07-10) — second erratum batch from Lane-1 dossier round 2 (23 further dossiers; primaries read): the "Genesis Tech" §5.1 row corrected to FTC v. *Growthmind/Wisey* (the brand is not a defendant; filed 2026-06-02, TRO 06-04); the EDPB erasure-framework status updated (report adopted 2026-02-10, investigations still open, no framework fine yet — ParkkiPate and APG predate it); §6's nst check-coverage claim **closed** — check 7 (reminders) gained its first real anchors via the CMA antivirus/console commitment mandates, so every nst check 1–7 is now real-anchored (checks 1/3/4/5/6 at fined strength, 2 at settlement, 7 at commitment). Export-side Art. 20 enforcement re-verified as still zero worldwide (third sweep).

- **0.1.1** (2026-07-10) — supply-survey errata from Lane-1 dossier research (round 1: 30 exhibit dossiers, primary records read; the two load-bearing corrections independently re-verified before this edit): **Adobe order entered** 2026-04-10 (§5.1 row + §8 soft spot resolved); **CNIL Google 2025 €325M is separate conduct** (SAN-2025-004: Gmail inline ads + account-creation cookie consent — §5.1 cluster corrected); **CFPB TransUnion dismissed with prejudice 2025-02** without relief (complaint strength only); §6's "0 found coverage of checks 5/6/7" **corrected** — real anchors exist for check-5 shapes (Wowcher, ABCmouse, TransUnion) and check 6 (AGCM eDreams PS12853, fined, still-ongoing conduct); check 7 remains authored-only. AGCM primaries partially readable via bollettino PDF paths. No demand-side or verdict changes.
