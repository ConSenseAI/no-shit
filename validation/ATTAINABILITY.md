...
|---|---|---|---|---|
| `no-subscription-trap` (behavioral) | reachable: ~30 exhibit-anchored re-implementations + synthetics | **~30 products** — *corrected 0.1.1:* checks **5 and 6** now have real anchors (check 5: Wowcher pre-tick, ABCmouse buried-terms consent, TransUnion missing affirmative consent; check 6: AGCM eDreams PS12853, fined 2026 — trial→charge with no advance notice, decision read primary); only **check 7** (reminders) remains authored-only | ~4–6 (Uber One; eDreams; UK Adobe probe; §312k reservoir pending own labeling) | reachable: platform-mandate class (store-scoped labels) + remediation-verified + construction |
...
- **`no-subscription-trap`:** the sealed violating set is roughly **half documented-real-anchored, half authored** — and the authored half is forced not by count but by *check coverage* (*narrowed 0.1.1:* auto-renew-consent and trial-conversion shapes now carry real anchors — Wowcher/ABCmouse/TransUnion and eDreams respectively; renewal-reminder facts remain undocumented at fixture grain anywhere). The clean set is attainable but every clean label carries a scope (store-mediated vs. web) and a date.
