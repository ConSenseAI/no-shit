...
- `no-dark-patterns@2.1` — formal definitions of manipulative UI patterns (confirmshaming, roach motel, privacy zuckering, etc.) with adversarial test cases drawn from published research (UIGuard, the Mathur corpus) and real-world examples.
...
**Design.** Assemble a labeled corpus of products with documented extractive behavior (FTC dark-pattern actions, CPPA enforcement targets, academically documented examples) and documented-clean controls, drawn from open-source and client-side software where ground truth is inspectable — sized per the protocol's floors (≥60 violating + ≥30 clean per criterion per measured cell, [`validation/PROTOCOL.md`](validation/PROTOCOL.md) §4.2, which supersede this document's earlier ~50/~50 sketch). Run the reference pipeline per criterion against the corpus, for both model pools (see confidentiality tiers below). Publish: per-criterion false-pass and false-flag rates, consensus-failure frequency, inter-model agreement, and measured token cost per audit. Behavioral probe accuracy (Architecture §7) is validated **inside the same study** — the behavioral cells plus the defeat-device red-team lane *are* the probe-accuracy measurement (PROTOCOL §3 Q2; probe observations sit much closer to ground truth than code judgment, but they get measured, not assumed).

**The corpus has a selection-bias problem, stated before it bites.** Documented extraction and inspectable ground truth are anti-correlated by construction: the well-documented extractive cases (FTC actions — Amazon Prime cancellation, Adobe termination fees; CPPA targets) are overwhelmingly closed, server-side SaaS, exactly what a code audit cannot see, while open-source and client-side software — where code ground truth is inspectable — systematically skews clean, which is nearly this document's own thesis. The intersection "documented-extractive AND code-inspectable" may be far thinner than 50 products. Handling, in order: (1) corpus attainability is audited *before* kill thresholds are finalized — **done** ([`validation/ATTAINABILITY.md`](validation/ATTAINABILITY.md), 2026-07-08): the anti-correlation is confirmed and counted — the documented-extractive-AND-code-inspectable intersection is roughly five real products worldwide — and the real slice is reported, not papered over; (2) the shortfall is filled with synthetic variants — extraction patterns from the documented closed-source cases re-implemented into otherwise-clean open codebases — labeled as synthetic and scored separately, so the external-validity caveat travels with the results (peer-reviewed support for the implant method: SusBench, IUI 2026); (3) client-side proprietary products with decompilable or observably-documented behavior (browser extensions, the UIGuard/AidUI screenshot corpora) fill the middle; (4) the audit's kill-branch check — "a credible code-auditable extractive corpus cannot be assembled at all" — did **not** trigger, but only via the sanctioned synthetic path: code-cell results generalize through the implant assumption, the behavioral tier carries the external-validity weight, and the report will say so. A kill-criteria *pass* on an unrepresentative corpus would be worse than a fail — it would launder the keystone assumption the study exists to test.
